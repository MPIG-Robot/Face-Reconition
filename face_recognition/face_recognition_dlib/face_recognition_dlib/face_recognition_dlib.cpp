// face_recognition_dlib.cpp : 定义控制台应用程序的入口点。
//
...
int main(int argc, char** argv) try
{
	...
	load_image(img1, argv[2]);
	// Display the raw image on the screen
	image_window win1(img1);

	std::vector<matrix<rgb_pixel>> faces1;
	for (auto face1 : detector(img1))
	{
		auto shape1 = sp(img1, face1);
	...
	cout << "number of people found in the database: " << face_descriptors.size() << endl;
	cout << "number of people found from the input: "  << face_descriptors1.size() << endl;


	if (face_descriptors.size() < 1)
	{
		cout << "Wrong face number detection in image 1" << endl;
	}
	if (face_descriptors1.size() > 1)
	{
		cout << "Wrong face number detection image2" << endl;
	}

	for (size_t i = 0; i < face_descriptors.size(); ++i)
	{
		if (length(face_descriptors1[0] - face_descriptors[i]) < 0.6)
		{
			cout << "faces found in image!" << endl;
			break;
		}
		else
		{
			cout << "No match faces found in image!" << endl;
		}
	}
	...
	cout << "number of people found in the image1: " << face_descriptors.size() << endl;
	cout << "number of people found in the image2: " << face_descriptors1.size() << endl;
	...
	{
		if (length(face_descriptors[0] - face_descriptors1[i]) < 0.6)
		{
			cout << "faces found in image!" << endl;
			break;
		}
